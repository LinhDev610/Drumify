...
Hệ thống sử dụng **Cloudinary SDK** để quản lý tài nguyên hình ảnh một cách chuyên nghiệp:

1. **Avatar Upload**: Cho phép người dùng cập nhật ảnh đại diện trực tiếp từ trang Profile.
2. **Server-side Upload**: File được gửi lên Backend (`MediaService`), sau đó mới được đẩy lên Cloudinary để đảm bảo bảo mật và dọn dẹp ảnh cũ tự động.
3. **Optimized Delivery**: Tự động tối ưu định dạng (WebP) và kích thước theo thiết bị người dùng.
